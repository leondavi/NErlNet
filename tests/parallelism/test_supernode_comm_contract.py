...
class SuperNodeCommContractTest(unittest.TestCase):
    def test_super_node_delivers_to_parallel_deliver_endpoint(self) -> None:
        ...
        self.assertIn("{parallel_deliver, DeliveryId, FromWorker, ToWorker, Data}", content)
        self.assertIn("parallel_deliver_ack", content)
        self.assertIn("maybe_retry_pending_parallel_deliveries", content)
    ...
    def test_client_has_parallel_deliver_path(self) -> None:
        ...
        self.assertIn("{parallel_deliver, DeliveryId, From, To, Data}", handler_content)
        self.assertIn("{parallel_deliver, FromWorker, ToWorker, Data}", statem_content)
        self.assertIn("handle_parallel_delivery_with_ack", statem_content)
        self.assertIn("parallel_delivery_seen_ids", statem_content)
        self.assertIn("atom_to_list(parallelDeliverAck)", statem_content)
        ...
        self.assertIn("\"/parallelDeliverAck\"", app_content)
