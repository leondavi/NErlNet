...
class TorchParallelNifContractTests(unittest.TestCase):
    ...
    def test_torch_erlang_bridge_exports_parallel_training_apis(self) -> None:
        ...
        self.assertIn("pipeline_stage0_forward_nif/4", content)
        self.assertIn("pipeline_stage_forward_nif/6", content)
        self.assertIn("pipeline_stage_last_forward_backward_nif/6", content)
        self.assertIn("pipeline_stage_backward_nif/4", content)
    ...
    def test_torch_native_nif_registers_parallel_symbols(self) -> None:
        ...
        self.assertIn('{"pipeline_stage0_forward_nif", 4, pipeline_stage0_forward_nif}', content)
        self.assertIn('{"pipeline_stage_forward_nif", 6, pipeline_stage_forward_nif}', content)
        self.assertIn('{"pipeline_stage_last_forward_backward_nif", 6, pipeline_stage_last_forward_backward_nif}', content)
        self.assertIn('{"pipeline_stage_backward_nif", 4, pipeline_stage_backward_nif}', content)
