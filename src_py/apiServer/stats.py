...
class Stats():
    ...
    def get_confusion_matrices_eeg(self , normalize : bool = False ,plot : bool = False , saveToFile : bool = False): 
        ...
        for source_piece_inst in sources_pieces_list:
            ...
            for worker_db in workers_model_db_list:
                ...
                if len(self.headers_list) == 1:   # One class
                    ...
                    #discritize the max_column_predict/labels_index to 3 bins: {0,1,2}-> 1, {3,4,5}->2, {6,7,8}->3
                    max_column_predict_index_bins = pd.cut(max_column_predict_index, bins=[-1, 2, 5, 8], labels=[0, 1, 2]).tolist()
                    max_column_labels_index_bins = pd.cut(max_column_labels_index, bins=[-1, 2, 5, 8], labels=[0, 1, 2]).tolist()
                    
                    # building confusion matrix for each class
                    bins_labels = [1, 2, 3]
                    for class_index, class_name in enumerate(bins_labels):
                        if has_multiple_max:
                            class_predict_list = [1 if class_index in row_max_list else 0 for row_max_list in max_column_predict_index_bins]
                        else:
                            class_predict_list = [1 if label_num == class_index else 0 for label_num in max_column_predict_index_bins]   # 1 if the label is belong to the class, 0 otherwise
                        class_actual_list = [1 if label_num == class_index else 0 for label_num in max_column_labels_index_bins]   # 1 if the label is belong to the class, 0 otherwise
                        labels = [0, 1]
                        confusion_matrix = metrics.confusion_matrix(class_actual_list, class_predict_list, labels=labels)  
                        confusion_matrix_source_dict[(source_name, worker_name, class_name)] = confusion_matrix
                        if (worker_name, class_name) not in confusion_matrix_worker_dict:
