...
def WinExperimentFlowDialog():
    ExperimentFlowDatasetLayout = [[sg.Text("csv file path: "),sg.In(enable_events=True ,key=WIN_EXPFLOW_KEY_CSV_FILE_PATH_CHOSEN_FILE, expand_x=True), sg.FileBrowse(file_types=(("csv file", "*.csv"),))],
                                   [sg.Text("Number of labels"),sg.InputText(key=WIN_EXPFLOW_KEY_NUM_OF_LABELS, enable_events=True)],
                                   [sg.Text("Number of features"),sg.InputText(key=WIN_EXPFLOW_KEY_NUM_OF_FEATURES, enable_events=True)],
                                   [sg.Text("First row headers"),sg.Checkbox("Yes", key=WIN_EXPFLOW_KEY_FIRST_ROW_HEADERS, enable_events=True)],
                                   [sg.Text("Headers names"),sg.InputText(key=WIN_EXPFLOW_KEY_HEADERS_NAMES, enable_events=True)]]
    ExperimentFlowDatasetFrame = sg.Frame("Dataset Settings",layout=ExperimentFlowDatasetLayout)
    
    ExperimentFlowAddPhaseLayout = [[sg.Text("Phase name"),sg.InputText(key=WIN_EXPFLOW_KEY_PHASE_NAME, enable_events=True)],
                                    [sg.Text("Starting sample"),sg.InputText(key=WIN_EXPFLOW_KEY_PHASE_STARTING_SAMPLE, enable_events=True)],
                                    [sg.Text("Number of batches"),sg.InputText(key=WIN_EXPFLOW_KEY_PHASE_NUM_OF_BATCHES, enable_events=True)],
                                    [sg.Text("Workers"),sg.InputText(key=WIN_EXPFLOW_KEY_PHASE_WORKERS, enable_events=True)],
                                    [sg.Text("Sources"),sg.InputText(key=WIN_EXPFLOW_KEY_PHASE_SOURCES, enable_events=True)],
                                    [sg.Button("Add", key=WIN_EXPFLOW_KEY_BUTTON_ADD_PHASE, enable_events=True)]]
    ExperimentFlowAddPhaseFrame = sg.Frame("Add Phase",layout=ExperimentFlowAddPhaseLayout)

    ExpermentFlowPhasesListLayout = [[sg.Text("Phases",expand_x=True)],
                                     [sg.Listbox(WinExperimentPhasesNamesList, size=(30,15), expand_x=True, key=WIN_EXPFLOW_KEY_PHASES_LIST_BOX, enable_events=True)]]
    ExperimentFlowPhasesListFrame = sg.Frame("Phases",layout=ExpermentFlowPhasesListLayout)

    ExperimentFlowPhaseFrame = sg.Frame("Phase",layout=[[ExperimentFlowAddPhaseFrame],[ExperimentFlowPhasesListFrame]])
    ...
    ExperimentFlowFileFrame = sg.Frame("File",layout=ExperimentFlowFileLayout)
