...
% start_connection() ->
%   inets:start(),
%   httpc:set_options([{proxy, {{"192.168.0.108", 8080},["192.168.0.108"]}}]),
%   httpc:set_options([{proxy, {{"192.168.0.113", 8080},["192.168.0.108"]}}]).
...
% training()->
%   httpc:request(post,{"http://192.168.0.113:8080/updateCSV", [],"application/x-www-form-urlencoded","s1,w1,RunOrWalkTrain_splitted"}, [], []),
%   httpc:request(post,{"http://192.168.0.113:8080/clientsTraining", [],"application/x-www-form-urlencoded",[]}, [], []).

% startCasting()-> ok.
% stop_training()->
%   httpc:request(post,{"http://192.168.0.113:8080/startCasting", [],"application/x-www-form-urlencoded","s1,1000"}, [], []).
