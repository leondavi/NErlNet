...
-record(workerNN_state, {clientPid, features, labels, myName, modelId, nextState, currentBatchID=0,ackClient=0,  missedSamplesCount = 0, missedTrainSamples= [],optimizer, lossMethod, learningRate}).
...
init({WorkerName,ModelId, ModelType, ScalingMethod,LayerTypesList,LayersSizes,LayersActivationFunctions,Optimizer, Features, Labels, LossMethod, LearningRate, ClientPID}) ->
...
%%^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  {ok, idle, #workerNN_state{clientPid = ClientPID, features = Features, labels = Labels, myName = WorkerName,
                                 modelId = ModelId,optimizer = Optimizer,learningRate = LearningRate, lossMethod = LossMethod}}.
...
state_name(_EventType, _EventContent, State = #workerNN_state{}) ->
...
handle_event(_EventType, _EventContent, _StateName, State = #workerNN_state{}) ->
...
code_change(_OldVsn, StateName, State = #workerNN_state{}, _Extra) ->
  {ok, StateName, State}.
...
%% State idle
idle(cast, {training}, State = #workerNN_state{myName = MyName,clientPid = ClientPid}) ->
  ?LOG_NOTICE("Go from idle to train!\n"),
  gen_statem:cast(ClientPid,{stateChange,MyName}),
  {next_state, train, State};

idle(cast, {predict}, State = #workerNN_state{myName = MyName,clientPid = ClientPid}) ->
  ?LOG_NOTICE("Go from idle to predict\n"),
  gen_statem:cast(ClientPid,{stateChange,MyName}),
  {next_state, predict, State#workerNN_state{nextState = predict}};

idle(cast, {set_weights,_Ret_weights_list}, State = #workerNN_state{modelId=_ModelId}) ->
...
% wait(cast, {loss, LossFunc}, State = #workerNN_state{clientPid = ClientPid, myName = MyName, nextState = NextState,ackClient = AckClient}) ->
...
%   {next_state, NextState, State#workerNN_state{ackClient = 0}};


%% Waiting for receiving results or loss function
%% Got nan or inf from loss function - Error, loss function too big for double
wait(cast, {loss,nan,Time_NIF}, State = #workerNN_state{clientPid = ClientPid, myName = MyName, nextState = NextState,ackClient = AckClient}) ->
...
wait(cast, {loss, {LossVal,Time}}, State = #workerNN_state{clientPid = ClientPid, myName = MyName, nextState = NextState, modelId=ModelID,ackClient = AckClient}) ->
...
wait(cast, {predictRes,NerlTensor, Type, TimeTook, CSVname,BatchID}, State = #workerNN_state{myName = MyName, clientPid = ClientPid, nextState = NextState,ackClient = AckClient}) ->
...
wait(cast, {idle}, State) ->
  ...
  {next_state, wait, State#workerNN_state{nextState = idle,ackClient=1}};

wait(cast, {training}, State) ->
  %logger:notice("Waiting, next state - train"),
  % gen_statem:cast(ClientPid,{stateChange,WorkerName}),
  {next_state, wait, State#workerNN_state{nextState = train,ackClient=1}};

wait(cast, {predict}, State) ->
  %logger:notice("Waiting, next state - predict"),
  {next_state, wait, State#workerNN_state{nextState = predict,ackClient=1}};

wait(cast, {sample, _SampleListTrain}, State = #workerNN_state{missedSamplesCount = MissedSamplesCount, missedTrainSamples = _MissedTrainSamples}) ->
  ...
  {next_state, wait, State#workerNN_state{missedSamplesCount = MissedSamplesCount+1}};

wait(cast, {sample,_CSVname, _BatchID, _SampleListPredict}, State = #workerNN_state{missedSamplesCount = MissedSamplesCount, missedTrainSamples = _MissedTrainSamples}) ->
...
train(cast, {sample, {<<>>, Type}}, State ) ->
  ...
  {next_state, train, State#workerNN_state{nextState = train}};
  
%% Change SampleListTrain to NerlTensor
train(cast, {sample, {NerlTensorOfSamples, Type}}, State = #workerNN_state{modelId = ModelId, optimizer = Optimizer, lossMethod = LossMethod, learningRate = LearningRate}) ->
    ...
    {next_state, wait, State#workerNN_state{nextState = train}};
  
%% TODO: implement send model and weights by demand (Tensor / XML)
train(cast, {set_weights,Ret_weights_list}, State = #workerNN_state{modelId = ModelId}) ->
  %% Set weights
  %io:format("####sending new weights to workers####~n"),
...
train(cast, {idle}, State = #workerNN_state{myName = MyName, clientPid = ClientPid}) ->
  %logger:notice("Go from train to idle\n"),
  gen_statem:cast(ClientPid,{stateChange,MyName}),
  {next_state, idle, State};

train(cast, {predict}, State = #workerNN_state{myName = MyName, clientPid = ClientPid}) ->
...
%% State predict
predict(cast, {sample,_CSVname, _BatchID, {<<>>, Type}}, State = #workerNN_state{}) ->
  ?LOG_ERROR("Received empty tensor"),
  {next_state, predict, State#workerNN_state{nextState = predict}};

% send predict sample to worker
predict(cast, {sample,CSVname, BatchID, {PredictBatchTensor, Type}}, State = #workerNN_state{ modelId = ModelId}) ->
    CurrPID = self(),
    _Pid = spawn(fun()-> nerlNIF:call_to_predict(ModelId,PredictBatchTensor, Type,CurrPID,CSVname, BatchID) end),
    {next_state, wait, State#workerNN_state{nextState = predict}};
  
predict(cast, {idle}, State = #workerNN_state{myName = MyName, clientPid = ClientPid}) ->
  %logger:notice("Go from predict to idle\n"),
  gen_statem:cast(ClientPid,{stateChange,MyName}),

  {next_state, idle, State};

predict(cast, {training}, State = #workerNN_state{myName = MyName, clientPid = ClientPid}) ->
