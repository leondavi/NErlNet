...
fTensor1DPtr AeRed::update_batch(fTensor2DPtr loss_values)
{
    fTensor1DPtr result = std::make_shared<fTensor1D>(loss_values->dimension(0));
    for(int i = 0; i < loss_values->dimension(0); i++)
    {
        float val = update_sample(loss_values->data()[i]);
        result->data()[i] = val;
    }
    return result;
}
