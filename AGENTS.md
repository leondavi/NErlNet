...
- Python worker communication stats now backfill per-worker `acc_time_*`, `average_time_*`, and `batches_sent_*` from phase result durations in model DB when runtime counters are zero (common in pipeline paths), improving report relevance without requiring immediate runtime stats schema changes.
